...
class StartupTNScraper:
    ...
    def log(self, level: str, message: str, page: int | None = None, company: str | None = None) -> None:
        ...
        try:
            session.execute(text("INSERT INTO logs (job_id, level, message, page, company, created_at) VALUES (:j,:l,:m,:p,:c,CURRENT_TIMESTAMP)"), {"j": self.job_id, "l": level, "m": message, "p": page, "c": company})
            session.commit()
    ...
    def update_job_progress(self, **values: Any) -> None:
        ...
        try:
            ...
            session.execute(text(f"UPDATE jobs SET {assignments}, updated_at = CURRENT_TIMESTAMP WHERE id = :job_id"), values)
            session.commit()
        except Exception as exc:
            session.rollback()
    ...
    def _upsert_company(self, data: dict[str, Any]) -> tuple[bool, bool]:
        ...
        try:
            ...
            if exists:
                session.execute(text(f"UPDATE companies SET {', '.join(f'{key}=:{key}' for key in fields)}, scraped_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE profile_url=:profile_url"), data)
                is_updated = True
            else:
                session.execute(text(f"INSERT INTO companies (profile_url, {', '.join(fields)}, scraped_at, updated_at) VALUES (:profile_url, {', '.join(f':{key}' for key in fields)}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"), data)
                is_created = True
            session.commit()
